...
async function setMemberRole(formData: FormData) {
  ...
  const { error } = await service
    ...
    .update({ role: newRole })
    .eq("id", memberId);
  if (error) console.error("setMemberRole error:", error);
...
export default async function AdminMembersPage() {
  ...
  if (service) {
    try {
      ...
      console.error("Service client error, falling back to profiles:", err);
    }
  }
            ...
            {members.map((member) => (
              <div key={member.id}
                className={`flex flex-col gap-4 rounded-[20px] border bg-[#030814] p-4 sm:flex-row sm:items-center sm:justify-between ${
                  member.isBanned ? "border-red-900/30" : "border-white/8"
                }`}>
                ...
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border text-sm font-semibold uppercase ${
                    member.isBanned
                      ? "border-red-900/30 bg-red-900/20 text-red-500"
                    ...
                    <>
                        ...
                        <form action={setMemberRole} className="flex flex-wrap gap-2">
                          ...
                          {member.role === "member" && (
                            <button name="new_role" value="premium"
                              className="rounded-2xl border border-orange-500/20 bg-orange-500/10 px-3 py-1.5 text-xs font-medium text-orange-300 hover:bg-orange-500/20">
                              Grant Premium
                            </button>
                          )}
                          {member.role === "premium" && (
                            <button name="new_role" value="member"
                              className="rounded-2xl border border-slate-500/20 bg-slate-500/10 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-500/20">
                              Revoke Premium
                            </button>
                          )}
                          {member.role !== "admin" && (
                            <button name="new_role" value="admin"
                              className="rounded-2xl border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-xs font-medium text-red-300 hover:bg-red-500/20">
                              Make Admin
                            </button>
                          )}
                          {member.role === "admin" && (
                            <button name="new_role" value="member"
                              className="rounded-2xl border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-slate-400 hover:bg-white/10">
                              Remove Admin
                            </button>
                          )}
                        </form>
                      ...
                      {!member.isBanned ? (
                        <form action={banMember}>
                          <input type="hidden" name="member_id" value={member.id} />
                          <button type="submit"
                            className="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 text-xs font-medium text-amber-300 hover:bg-amber-500/20">
                            Ban
                          </button>
                        </form>
                      ) : (
                        <form action={unbanMember}>
                          <input type="hidden" name="member_id" value={member.id} />
                          <button type="submit"
                            className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-300 hover:bg-emerald-500/20">
                            Unban
                          </button>
                        </form>
                      )}
